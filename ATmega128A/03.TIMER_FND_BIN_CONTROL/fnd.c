...
#define RUN 1
#define RESET 2
extern volatile uint32_t button_delay_counter;
volatile uint32_t button_delay_counter = 0;
...
volatile uint8_t dot_display = 0; //Dot 제어 

volatile uint32_t sw_ms_count = 0;
volatile uint32_t sw_sec_count = 0;

volatile uint8_t mode = CLOCK; //시계 모드 저장(초기값:분.초 시계)
volatile uint8_t stopwatch = STOP; //스톱워치 상태 저장(초기값:정지)
volatile uint8_t circle_index = 0; //초시계 원 위치 저장
volatile uint32_t circle_ms = 0; //초시계 원 ms 카운터

ISR(TIMER2_OVF_vect) {
	TCNT2 = 194;
	fnd_display();
	ms_count++;
	if (button_delay_counter < 30) {
