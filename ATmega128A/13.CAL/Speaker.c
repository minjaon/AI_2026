...
#define FREQ_1KHz   999
#define FREQ_2KHz   499
#define FREQ_3KHz   332
#define FREQ_4KHz   249
#define NOTE_DO    3830 // 261Hz
#define NOTE_MI    3038 // 329Hz
#define NOTE_SO    2550 // 392Hz
#define NOTE_DO2   1914 // 523Hz
#define NOTE_MI2   1519 // 659Hz
#define NOTE_SO2   1274 // 784Hz
#define FREQ_261Hz  3830
#define FREQ_329Hz  3038
#define FREQ_392Hz  2550
#define FREQ_554Hz  1804
#define DO_01   1911
#define DO_01_H 1817
#define RE_01   1703
#define RE_01_H 1607
...
int Airplane_Tune[] = {
    // 떴다 떴다 비행기
    MI_01, RE_01, DO_01, RE_01, MI_01, MI_01, MI_01,
    // 날아라
    RE_01, RE_01, RE_01,
    // 날아라
    MI_01, SO_01, SO_01,
    ...
    MI_01, RE_01, DO_01, RE_01, MI_01, MI_01, MI_01,
    // 우리 비행기
    RE_01, RE_01, MI_01, RE_01, DO_01, '/0'
};

const int Airplane_Beats[] = {
    // 떴다 떴다 비행기
    BEAT_1_4, BEAT_1_4, BEAT_1_4, BEAT_1_4, BEAT_1_4, BEAT_1_4, BEAT_1_2,
    // 날아라
...
int PowerOn_Tune[] = {FREQ_1KHz, FREQ_2KHz, FREQ_3KHz, FREQ_4KHz, 0, '/0'};
int PowerOn_Beats[] = {70, 70, 70, 70, 3000};

// 2. open_buzzer 악보
int OpenBuzzer_Tune[] = {FREQ_261Hz, FREQ_329Hz, FREQ_392Hz, FREQ_554Hz, 0, '/0'};
int OpenBuzzer_Beats[] = {70, 70, 70, 70, 3000};
	

void Play_Melody_Array(int *tone, int *beats)
{
	while(*tone != 0)
	{
		if (stop_flag) {
			DDRD &= ~(1 << PORTD6); // 핀 입력 전환 (음소거)
			OCR3A = 0;              // 타이머 비교값 초기화
			return;
		}	
		OCR3A = *tone; 
		
		int total_duration = *beats;

		int sound_time = (total_duration * 65) / 100;
		int mute_time = total_duration - sound_time;
		
		DDRD |= (1 << PORTD6); 
		for (int i = 0; i < sound_time / 10; i++) {
			if (stop_flag) { DDRD &= ~(1 << PORTD6); OCR3A = 0; return; }
			_delay_ms(10);
		}
		
		DDRD &= ~(1 << PORTD6); 
		for (int i = 0; i < mute_time / 10; i++) {
			if (stop_flag) { OCR3A = 0; return; }
			_delay_ms(10);
		}
...
int DrumStop_Tune[] = {
	// 1줄: 레 솔 파# 미 레 시
	M_D6, M_G5, M_FS6, M_E6, M_D6, M_B5,
	// 2줄: 도 레 미 라 시 도 시 레
	M_C6, M_D6, M_E6, M_A5, M_B5, M_C6, M_B5, M_D6,
	// 3줄: 레 솔 파# 미 레 솔
	M_D6, M_G5, M_FS6, M_E6, M_D6, M_G5,
	// 4줄: 솔 라 솔 파# 미 파# 솔
	M_G5, M_A5, M_G5, M_FS6, M_E6, M_FS6, M_G5,
	0 
};

int DrumStop_Beats[] = {
	BEAT_1_4, BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_4, BEAT_1_4,
	BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_4, BEAT_1_8,
	BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_8, BEAT_1_4, BEAT_1_4,
