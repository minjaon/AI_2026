...
{
	Macro_Set_Bit(RCC->APB1ENR, 2); //TIM4에 클럭(전원)을 공급

	TIM4->CR1 = (1<<4)|(0<<3); //다운 카운팅(Down-counting) 모드
	TIM4->PSC = (unsigned int)(TIMXCLK/TIM4_FREQ + 0.5)-1; //타이머가 초당 몇 번 동작할지
	TIM4->ARR = TIME4_PLS_OF_1ms * time - 1; //레지스터에 타이머가 카운트할 시작 값

	Macro_Set_Bit(TIM4->EGR,0);
	Macro_Clear_Bit(TIM4->SR, 0);
	Macro_Set_Bit(TIM4->CR1, 0); //타이머 카운터(CEN, Counter Enable)를 작동
}

int TIM4_Check_Timeout(void) //타임아웃이 발생했는지 확인
{
...
void TIM3_Out_PWM_Generation(unsigned short freq,int a)
{
	TIM3->PSC = (unsigned int)(TIMXCLK/TIM3_FREQ + 0.5)-1; //클럭을 몇 번 칠 때마다 타이머 카운터를 1 올릴지 결정하는 분주비
	// 요청한 주파수가 되도록 ARR 설정
	TIM3->ARR = (TIM3_FREQ/freq) - 1 ; //카운터가 몇까지 세고 0으로 리셋(알람)될지 정하는 목표값
	TIM3->CCR3 = (unsigned int)((TIM3->ARR + 1) * (a / 100.0));
