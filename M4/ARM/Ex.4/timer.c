...
{
    int i;
    unsigned int t = TIM2_1ms_Pls * time; // 1ms당 클록 펄스 수를 곱해 총 펄스 수 계산
...
void TIM5_Out_PWM_Generation(unsigned short freq, uint8_t duty)
{
	//a0, a1= 1이면 PWM 동작(MODER에 0x2), 0이면 GPIOA 출력(MODER에 0x1)
	Macro_Write_Block(GPIOA->MODER, 0xf, 0x2, 10);
